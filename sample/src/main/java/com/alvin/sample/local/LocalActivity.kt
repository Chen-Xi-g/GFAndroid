 ...
 * @author 高国峰
 */
class LocalActivity : BaseVMActivity<ActivityLocalBinding, LocalVM>(
    R.layout.activity_local
) {
    /**
     * 标题布局
     */
...
/*        titleLayout<CustomTitleLayoutBinding> {
    ...
    override fun setStatusBar(activity: Activity) {
        // 设置状态栏颜色
        BarUtils.setStatusBarColor(activity, ContextCompat.getColor(activity, R.color.purple_500))
        // 设置导航栏颜色
        BarUtils.setNavBarColor(activity, ContextCompat.getColor(activity, R.color.purple_500))
        // 设置状态栏图标颜色
