...
class MainViewModel : BaseViewModel() {
    ...
    fun getFail() {
        viewModelScope.launch {
            httpCallback.beforeNetwork.value = NetworkModel(isShowDialog = true)
            delay(3000)
            _testData.value = null
            httpCallback.failed.value = NetworkModel(isShowErrorLayout = true, msg = "网络异常")
        }
    }
    ...
    fun getEmpty() {
        viewModelScope.launch {
            httpCallback.beforeNetwork.value = NetworkModel()
            delay(3000)
            _testData.value = listOf()
            httpCallback.afterNetwork.value = false
        }
    }
