...
import com.alvin.gfad.utils.DefaultDecoration.Edge.Companion.computeEdge
 ...
 * RecyclerView 分割线工具
 *
 * [引用](https://github.com/liangjingkanji/BRV)
 *
...
class DefaultDecoration constructor(private val context: Context) : RecyclerView.ItemDecoration() {
    ...
    private var onEnabled: (ReuseAdapter.BaseViewHolder.() -> Boolean)? = null
     ...
     */
    fun onEnabled(block: ReuseAdapter.BaseViewHolder.() -> Boolean) {
    ...
    override fun getItemOffsets(
        ...
        onEnabled?.let {
            val vh = parent.findContainingViewHolder(view) as ReuseAdapter.BaseViewHolder
            val itemOrNull = vh.getItemOrNull<Any>()
            if (!expandVisible && itemOrNull != null && itemOrNull is ItemExpand && itemOrNull.itemExpand) return
    ...
    private fun drawHorizontal(canvas: Canvas, parent: RecyclerView, reverseLayout: Boolean) {
        ...
        loop@ for (i in 0 until parent.childCount) {
            ...
            if (onEnabled != null) {
                val vh = parent.getChildViewHolder(child) as ReuseAdapter.BaseViewHolder
                val itemOrNull = vh.getItemOrNull<Any>()
                if (!expandVisible && itemOrNull != null && itemOrNull is ItemExpand && itemOrNull.itemExpand) continue@loop
